...
export default function Card({ingredient, onQuantityChange, initialQuantity = 0 }) {
    ...
    <div>
        <div  className="w-[300px] h-[300px] px-4 py-8 bg-white rounded-2xl shadow-md">
          ...
          <div>
            ...
            <p className="font-bold text-xl">{ingredient.calories}</p><p className="text-[#F8B602] font-bold text-xl">Cal</p>
            </div>
          </div>
          <div className="flex justify-end items-center gap-4">
             {quantity > 0 && (
              <button onClick={() => updateQuantity(-1)}>
              <Image width={30} height={30} src="./svg/buttonminus.svg" alt="Decrease" />
              </button>
              )}
              {quantity > 0 && ( <p className="font-bold text-2xl">{quantity}</p>)}

             <button onClick={() => updateQuantity(1)}>
             <Image width={30} height={30} src="./svg/buttonplus.svg" alt="Increase" />
            </button>
          </div>
        </div>   
    </div>
